...
export function Conditions({ocean, weather}: {ocean: Ocean; weather: Weather}) {
  return <div className="conditions-grid">
    <section className="panel"><div className="panel-title"><div><span className="eyebrow">Nearby Atlantic observations</span><h2>Ocean conditions</h2></div>{ocean.is_mock && <span className="mock-badge">Demo data</span>}</div>
      ...
      <small>NOAA/NDBC Station {ocean.station_id} · Hollywood Beach, FL{ocean.status_message && ` · ${ocean.status_message}`}</small>
    </section>
    <section className="panel"><div className="panel-title"><div><span className="eyebrow">Deerfield Beach Pier</span><h2>Local weather</h2></div>{weather.is_mock && <span className="mock-badge">Demo data</span>}</div>
